...
class Book:
    def __init__(self, isbn, title, author, genre, copies, book_type):
        ...
        self.total_copies = copies  # to help in bulk acquisition tracking
...
class LibrarySystem:
    ...
    def add_book(self, isbn, title, author, genre, copies, book_type):
        if isbn not in self.books:
            ...
            self.books[isbn].total_copies += copies  # for bulk acquisitions
    ...
    def handle_waitlist(self, book, isbn):
        while book.waitlist and book.copies > 0:
            next_member = book.waitlist[0]
            print(f"Member {next_member}, '{book.title}' is available now.")
            response = input("Do you want to borrow it? (yes/no): ").strip().lower()
            if response == "yes":
                book.waitlist.pop(0)
                self.issue_book(next_member, isbn, "default")
            else:
                print(f"Member {next_member} declined.")
                book.waitlist.pop(0)
    ...
    def analyze_reading_patterns(self, member_id):
        ...
        genre_percent = {genre: round((count / total) * 100) for genre, count in genre_count.items()}
    ...
    def display_member_profile(self, member_id):
        ...
        print("Favorite Genres:", ', '.join(f"{g} ({p}%)" for g, p in genre_percent.items()))
        ...
        trending = self.generate_popular_books_report("monthly", list(genre_percent.keys())[0] if genre_percent else "")
...
if __name__ == "__main__":
    ...
    lib.return_book("LM001", "112", datetime.date.today(), "damaged")  # test damaged case

    lib.issue_book("LM001", "113", "BR001")
    lib.return_book("LM001", "113", datetime.date.today(), "lost")  # test lost case
